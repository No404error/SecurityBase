package cn.edu.sut.informationSecruity23.operation.user;

import cn.edu.sut.informationSecruity23.*;
...
import cn.edu.sut.informationSecruity23.param.UserParam;
...
public class UserOperation {
    ...
    public static Proof Show(AggregationCredential credential, String message) {
        ...
        //Element c = ElementOperation.Hash(phi_1,D1);
        ...
        Element D2_r11 = pairing.pairing(phi_1.duplicate().powZn(W_f.duplicate().negate()), g2).getImmutable();
        Element temp_var=pairing.pairing(phi_1.duplicate().powZn(c.duplicate().mul(fr)), g2).getImmutable();
        Element temp_res=D2_r11.duplicate().mul(temp_var).getImmutable();

        if(D2_r1==temp_res)
            System.out.println("D2_r1==temp_var");

        Element D2_r21 = g__1_E_g2.duplicate().powZn(W_k).getImmutable();
        Element D2_r31 =g1_E_g2.duplicate().powZn(W_u).getImmutable();
        Element D2_r41 = rtvk_E_rvk4.duplicate().powZn(W_o).getImmutable();
        Element D2_r51 = rtvk_E_g2.duplicate().powZn(W_beta).getImmutable();
        Element D2_r6_11 = pairing.pairing(phi_1, rvk[3]).duplicate().powZn(c.duplicate().negate()).getImmutable();

        //Element D2_r6 = (D2_r6_1.duplicate().powZn(c_negate).mul(g_1_E_g2.duplicate().powZn(c))).getImmutable();

        Element D2_r6 = pairing.pairing(PublicParam.g_1, g2).duplicate().powZn(c).getImmutable();
        Element D_2 = D2_r11.duplicate().mul(D2_r21).mul(D2_r31).mul(D2_r41).mul(D2_r51).mul(D2_r6).mul(D2_r6_11).getImmutable();


        if(D_2.isEqual(D2))
            System.out.println("D2");

        return new Proof(phi_1,phi_2,phi_3,credagg_ran,c,W_u,W_f,W_k,W_o,W_beta,W_ou,attr, message,D1,D2,D3,D4);
